...
export function ProjectOverview({ project, unreadCount, onOpen }: { project: Project; unreadCount: number; onOpen: (tab: string, song?: { listId: string; songId: string }) => void }) {
  ...
  const upcoming = events.filter(event => new Date(event.startsAt || event.date).getTime() >= now).sort((a,b) => new Date(a.startsAt || a.date).getTime() - new Date(b.startsAt || b.date).getTime()).slice(0,3);
